...
const ProjectsSection = () => {
  ...
  const visibleProjects = showAll 
    ? filtered 
    : filtered.filter(p => p.tier === 1 || p.tier === 2).slice(0, 7);
    ...
    <SectionBlock id="projects" title="Projects">
      ...
      <div className="flex flex-wrap gap-2 mb-10">
        {FILTERS.map((f) => {
          const count =
            f.key === 'all'
              ? PROJECTS.length
              : PROJECTS.filter((p) =>
                  p.categories.includes(f.key as Exclude<FilterKey, 'all'>),
                ).length;
        ...
        <span className="ml-auto self-center font-mono text-[10px] text-foreground/40 uppercase tracking-widest hidden sm:block">
          {visibleProjects.length} / {filtered.length} shown
        </span>
      </div>

      {/* ── Project Grid ── */}
      <div ref={gridRef} className="flex overflow-x-auto snap-x snap-mandatory gap-4 md:gap-6 pb-12 pt-6 px-4 -mx-4 scroll-pl-4 md:scroll-pl-0 no-scrollbar md:mx-0 md:px-0 md:pt-0 md:grid md:grid-cols-2 lg:grid-cols-3 md:overflow-x-visible md:snap-none">
      ...
      <div className="mt-8 flex flex-col sm:flex-row gap-6 items-start sm:items-center">
        {!showAll && filtered.length > visibleProjects.length && (
          <button
            onClick={() => {
              playClick();
              setShowAll(true);
            ...
            onMouseEnter={playHover}
            className="px-6 py-3 bg-black text-white font-mono text-xs font-black uppercase tracking-widest hover:bg-black/80 transition-colors"
          >
            Explore All Work
          </button>
        )}
